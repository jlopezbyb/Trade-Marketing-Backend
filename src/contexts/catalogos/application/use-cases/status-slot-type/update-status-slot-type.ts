import { StatusSlotEntity } from '@src/contexts/catalogos/core/entities/status-slot-type-entity';
import { StatusSlotTypeCatalogRepository } from '@src/contexts/catalogos/core/repositories/statys-slot-type-catalog-repository';
import { AppError } from '@src/contexts/shared/infrastructure/exception/AppError';

export class UpdateStatusSlotTypeUseCase {
  constructor(private readonly statusSlotTypeCatalogRepository: StatusSlotTypeCatalogRepository) {}

  async run(id: string, data: { name: string; description: string; isActive: boolean }) {
    const statusType = await this.statusSlotTypeCatalogRepository.getById(id);

    if (!statusType) throw new AppError('VEHICLE_TYPE_NOT_FOUND', 404, 'Vehicle type not found', true);

    const statusTypeUpdated = StatusSlotEntity.fromPrimitives({
      ...statusType.toPrimitives(),
      name: data.name,
      description: data.description,
      isActive: data.isActive
    });

    await this.statusSlotTypeCatalogRepository.update(statusTypeUpdated);
  }
}
